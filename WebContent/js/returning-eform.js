...
var mediaText = alasql('SELECT * \
		...
		WHERE returning.return_id LIKE ?', [ q3 ])[0];
//console.log(JSON.stringify(mediaText));
$('#span-return_id').text(''+ mediaText.returning.return_id +'');
$('#span-supplier_name').text(mediaText.supplier.name);
$('#span-whouse').text(mediaText.whouse.name);
var dat = [];
dat = mediaText.returning.del_date.split('-');
$('#span-del_date').text(dat[0] +'年'+ dat[1] +'月'+ dat[2] +'日');
// 隠しinputの作成
$('#return_id').val(mediaText.returning.return_id);//現状使っていない。
$('#supplier_name').val(mediaText.supplier.name);//現状使っていない。
$('#del_date').val(mediaText.returning.del_date);
// 隠し選択ボックスの作成
...
//	console.log(dat);
	alasql('UPDATE returning SET del_date = ? WHERE return_id LIKE ?', [ dat, q3 ]);
...
var sql = 'SELECT * \
	FROM returning \
	JOIN returning_detail ON returning_detail.return_id = returning.return_id \
	JOIN item ON item.id = returning_detail.item \
	JOIN kind ON kind.id = item.kind \
	WHERE returning.return_id LIKE ? ';
...
//		console.log(JSON.stringify(test));
		...
		if(amount_difference > 0){
			alasql('INSERT INTO trans VALUES(?,?,?,?,?,?)', [ trans_id, stock_id, del_date, - amount_difference, balance - amount_difference, supplier_name + 'へ返品予定の在庫へ追加' ]);
		} else if(amount_difference < 0){
			alasql('INSERT INTO trans VALUES(?,?,?,?,?,?)', [ trans_id, stock_id, del_date, - amount_difference, balance - amount_difference, supplier_name + 'へ返品予定の在庫から削除' ]);
		}
		// returningの更新（総額の更新）
		var all_price = alasql('SELECT SUM(price) AS price FROM returning_detail WHERE return_id LIKE ?', [ q3 ])[0];
		console.log(all_price);
		alasql('UPDATE returning \
				SET price = ? \
				WHERE return_id LIKE ?', [ all_price.price, q3 ]);
//		var test = alasql('SELECT * FROM returning WHERE return_id LIKE ?', [ q3 ]);
//		console.log(JSON.stringify(test));
		$(this).toggleClass('hidden');
		this_tr.find('span#amount').toggleClass('hidden');
...
$('.rowDelete').on('click',function(){
	...
	function rowDelete(e){
		...
		// transレコード追加
		var trans_id = alasql('SELECT MAX(id) + 1 as id FROM trans')[0].id;
		alasql('INSERT INTO trans VALUES(?,?,?,?,?,?)', [ trans_id, stock_id, del_date, amount, balance + amount, supplier_name + 'へ返品予定を取り消し' ]);
		// returningを更新
		var price = alasql('SELECT SUM(price) AS price FROM returning_detail WHERE return_id LIKE ?', [ q3 ])[0].price; 
		alasql('UPDATE returning SET price = ? WHERE return_id LIKE ?', [ price, q3 ])
		// ページの読み込み
		window.location.reload(true);
	}
	// ダイアログ内のnoボタンのイベント
...
//		console.log("hello")
		...
		if(item && amount){
			...
			// returningへの保存（金額の変更）
			var old_price = alasql('SELECT price FROM returning WHERE return_id LIKE ?', [ q3 ])[0].price;
//			console.log(JSON.stringify(price));
			alasql('UPDATE returning \
					SET price = ? \
					WHERE return_id LIKE ?', [ old_price + price, q3 ]);
		}
		// trの削除
		input_tr.remove();
		// ページの読み込み
